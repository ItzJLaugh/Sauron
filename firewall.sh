...
iptables -A INPUT -p tcp -m tcp --dport 4477 -m state --state NEW,ESTABLISHED -j ACCEPT
...
iptables -A INPUT -p tcp -m tcp --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A INPUT -j LOG --log-prefix "IDS-DROP " --log-level 4

#set output
iptables -A OUTPUT -o lo -j ACCEPT
iptables -A OUTPUT -p tcp -m tcp --sport 4477 -m state --state NEW,ESTABLISHED -j ACCEPT
...
iptables -A OUTPUT -m hashlimit --hashlimit-upto 2/sec --hashlimit-burst 2 --hashlimit-mode srcip --hashlimit-name ICMP_LIMIT -j ACCEPT
